...
typedef NS_ENUM(NSUInteger, SDImageCoderDecodeSolution) {
    ...
    /// 根据图像格式、硬件和OS版本自动选择解决方案。这可在兼容性和性能之间保持平衡。SDWebImage 5.13.0之后默认
    ...
    /// 始终使用CoreGraphics在位图上下文上绘制和触发解码。最佳兼容性。SDWebImage 5.13.0之前默认
    ...
    /// 在iOS/tvOS 15+上可用,使用UIKit的新CGImageDecompressor/CMPhoto进行解码。性能最佳。如果失败,也会回退到CoreGraphics
