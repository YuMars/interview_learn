...
__attribute__((noinline)) static MainFunc prepare(APIs& state, const MachOAnalyzer* dyldMH)
{
    ...
    // 如果使用了 DYLD_PRINT_SEARCHING，则列出禁用的内容
    ...
    if ( mainSet != nullptr ) {
        ...
        state.loaded.reserve(state.initialImageCount()); // help avoid reallocations of Vector// 避免 Vector 的重新分配
    }
#endif // SUPPORT_PREBUILTLOADERS
    if ( mainLoader == nullptr ) {
        // if no pre-built Loader, make a just-in-time one
        // 如果没有预构建加载器，则创建just-in-time加载器
        state.loaded.reserve(512);  // guess starting point for Vector size// 估计 Vector 的初始大小
    ...
    // 将插入的库移到主可执行文件之前，以确保正确的平面命名空间查找
    ...
    depOptions.rpathStack      = &loadChainMain;
#if SUPPORT_PREBUILTLOADERS
    // 如果我们找到了缺少的路径，则意味着我们找到了目标的更合适的路径
    ...
    // 主动的 weakDefMap 意味着我们在进行任何绑定之前构建 weakDefMap
    ...
    // do fixups
    {
        ...
        // C++ 规范规定，主可执行文件可以定义非弱函数，这些函数会覆盖 dylibs 中的 weak-defs。
        // 对于启动时绑定的内容，这种情况会自动发生，但 dyld 缓存是预绑定的，所以我们需要修补任何被主可执行文件中的非弱定义覆盖的绑定。
        // 注意，在 macOS 上，我们还允许 dylibs 拥有 weak-defs 的非弱覆盖。
    ...
    // 如果存在 interposing，则将 interpose 元组应用到 dyld 缓存中
    ...
    // 如果 mainLoader 是预构建的，则 dyld 缓存中可能会存在 weak-defs 的覆盖
    if ( mainLoader->isPrebuilt ) {
        ...
        // 解决方法，使其在 cacheWeakDefFixup 中可访问
    ...
    if ( kdebug_is_enabled(KDBG_CODE(DBG_DYLD, DBG_DYLD_UUID, DBG_DYLD_UUID_MAP_A)) ) {
        ...
        // 在我们解除磁盘上的 dyld 映射时已经发送了 dyld 在缓存中的事件
    ...
    // 程序变量（例如 environ）通常在 libdyld.dylib 中定义（但对于旧的 macOS 二进制文件，可能在主可执行文件中定义）
    // 记住程序变量的位置，以便 libc 可以同步它们
    ...
    *state.vars->environPtr    = (const char**)state.config.process.envp;
    ...
    // 填写 ProgramVars 的 ExclaveKit 部分，以传递给 Libsystem 的初始化程序
    ...
    // 如果以 JustInTimeLoader 启动，可能需要序列化它
    if ( needToWritePrebuiltLoaderSet ) {
    ...
    // 将延迟加载的 dylibs 分离到 delayLoaded 向量中
    ...
    if ( state.config.process.mainExecutable->hasProgramVars(progVarsOffset, crtRunsInitializers, dyldLookupFuncAddr) ) {
        ...
        // 这是旧的 macOS 应用程序，它有自己的 NXArgv 等全局变量。我们需要使用它们。
    ...
    if ( state.config.process.platform == dyld3::Platform::driverKit ) {
        ...
        // DriverKit 使用与 dyld 预期不同的多样性对指针进行签名。
    ...
    if ( usesCRT ) {
        ...
        // 主可执行文件使用 LC_UNIXTHREAD，dyld 需要调整内核参数栈并跳转到 "start"
#if SUPPPORT_PRE_LC_MAIN
        // backsolve for KernelArgs (original stack entry point in _dyld_start)
        // 回溯 KernelArgs（_dyld_start 中的原始栈入口点）
...
{
    // 发送调试追踪点，以指示 dyld 引导已开始 <rdar://46878536>
#if !TARGET_OS_EXCLAVEKIT
...
#if SUPPPORT_PRE_LC_MAIN
    // 旧的 macOS 二进制文件会重置堆栈并跳入 crt1.o glue，因此 RuntimeLocks 不能在堆栈中分配
    // 我们不能使用 "static RuntimeLocks locks;" 因为编译器会生成初始化程序或保护机制
...
#else
    // 堆栈分配 RuntimeLocks。它们不能在分配器池中分配，因为池通常是只读的
    RuntimeLocks locks;
#endif // SUPPPORT_PRE_LC_MAIN
    ...
    bootStrapMemoryManager.withWritableMemory([&] {
        ...
        // 为 ExternallyViewableState 设置初始状态
#if !TARGET_OS_EXCLAVEKIT
...
#if TARGET_OS_EXCLAVEKIT
    // 通知 liblibc_plat 所有静态初始化程序已运行，并让它完成进程启动
    state->vars->finalize_process_startup(appMain);

    // 如果我们到达这里，意味着 finalize_process_startup 返回了（它不应该返回）
...
#if TARGET_OS_OSX && !TARGET_OS_EXCLAVEKIT
    // <rdar://74518676> libSystemHelpers 未针对模拟器设置，因此直接调用 _exit()
    if (MachOFile::isSimulatorPlatform(state->config.process.platform))
        _exit(result);
#endif // TARGET_OS_OSX && !TARGET_OS_EXCLAVEKIT
